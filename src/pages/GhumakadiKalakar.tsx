...
const GhumakadiKalakar = () => {
  const navigate = useNavigate();
  const [eventTypeId, setEventTypeId] = useState<string | null>(null);
  const [selectedCity, setSelectedCity] = useState<string>("");

  // Fetch event types to get the ID for 'Ghumakari Kalaakar'
  ...
  useEffect(() => {
    if (eventTypes.length > 0) {
      const ghumakari = eventTypes.find(
        (et) => et.name.toLowerCase() === "ghumakari kalaakar"
      );
  ...
  const {
    ...
    queryKey: ["ghumakadi-kalakar-events", eventTypeId],
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#F26522' }}>
      {/* Orange Navbar with Ghumakari Kalaakar Logo */}
      <Navbar selectedCity={selectedCity} setSelectedCity={setSelectedCity} bgColor="#F26522" logoSrc="/ghumakarilogo.png" />
      <main className="flex-grow pt-24 pb-20 md:pb-0">
        ...
        <div className="flex flex-col items-center justify-center z-10 mt-8 mb-8">
          <img
            src="/ghumakarilogo.png"
            alt="Ghumakari Kalaakar Logo Center"
        ...
        <div className="container-padding py-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold mb-4 text-white">Ghumakari Kalaakar Events</h1>
            <p className="text-white max-w-2xl mx-auto">
              The Ghumakari Kalaakar experience is all about celebrating the spirit of wandering artists. Enjoy unique performances, soulful journeys, and creative expressions. <br />
              For us, you are a Ghumakari Kalaakar, a traveler and creator at heart.
            </p>
          </div>
          {/* Event List */}
          {loadingTypes || loadingEvents || fetchingEvents ? (
            ...
            <div className="text-center py-16 text-white text-xl font-semibold">No upcoming Ghumakari Kalaakar events yet. Check back soon!</div>
          ) : (
                  ...
                  <div className="bg-[#F7E1B5] rounded-3xl px-8 py-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {dateEvents.map((event, index) => (
                        <Card key={event.id} className="hover-scale border-none shadow-soft overflow-hidden" style={{ backgroundColor: '#F26522' }}>
                          ...
                          <CardFooter className="px-5 pb-5 pt-0 flex justify-between items-center text-white">
                            ...
                            <Button size="sm" variant="outline" className="bg-[#F7E1B5] text-[#F26522] border-none hover:bg-[#e6d7a8] font-bold" onClick={() => navigate(`/event/${event.id}`)}>
                              View Details
                            </Button>
                          </CardFooter>
                        </Card>
...
export default GhumakadiKalakar; 