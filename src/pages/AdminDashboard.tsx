...
const AdminDashboard = () => {
  ...
  const [isEventTypeEditDialogOpen, setIsEventTypeEditDialogOpen] =
    useState(false);
  const [isBannerEditDialogOpen, setIsBannerEditDialogOpen] = useState(false);
  const [isTestimonialEditDialogOpen, setIsTestimonialEditDialogOpen] =
    useState(false);
  ...
  const { data: pendingJoinRequests = [], isLoading: pendingRequestsLoading } =
  ...
  const handleApproveJoin = async (invitationId: string) => {
    const { error } = await updateInvitationStatus(invitationId, "accepted");
    if (error) {
      toast({
        title: "Error",
      ...
      toast({
        title: "Approved",
        description: "The user may now book the event.",
      });
  ...
  const handleRejectJoin = async (invitationId: string) => {
    const { error } = await updateInvitationStatus(invitationId, "declined");
    if (error) {
      toast({
        title: "Error",
  ...
  const {
    ...
    isLoading: pendingTestimonialsLoading,
  } = useQuery({
  ...
  const { data: hostInvitations = [], isLoading: hostInvitationsLoading } =
    useQuery({
  ...
  const handleProcessCompletedEvents = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          "An error occurred while processing completed events. Please check the console for details.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleTestCompletedEvents = async () => {
    try {
      ...
      toast({
        ...
        } completed events. Attendance fields: ${
          attendanceFieldsExist ? "OK" : "Missing"
  ...
  const handleDownloadSystemReport = async () => {
    try {
      ...
      csvContent.push(
        "Event ID,Title,Category,Date,Time,Venue,City,Price (₹),Status"
      );
      ...
      csvContent.push(
        "Booking ID,Event,Customer Name,Email,Phone,Tickets,Amount (₹),Status,Booking Date"
      );
      ...
      Object.entries(customerSpending)
        ...
        .forEach(([customer, data]) => {
          csvContent.push(
            `"${customer}",${data.total.toLocaleString()},${data.bookings}`
          );
        });
      csvContent.push("");
      ...
      link.href = url;
      link.download = `motojojo-system-report-${
    ...
    async function fetchHostActivity() {
      ...
      const activity = events
        ...
        .map((event) => {
          ...
          const eventBookings =
            bookings?.filter((b) => b.event_id === event.id) || [];
          const eventTickets =
            tickets?.filter((t) =>
              eventBookings.map((b) => b.id).includes(t.booking_id)
            ) || [];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
        ...
        <div className="container-padding">
          ...
          >
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription className="text-black">
                      Review and approve or reject pending requests to join
                      private events
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <TableBody>
                          {pendingJoinRequests.map((req) => (
                            <TableRow key={req.id}>
                              ...
                              <TableCell>
                                {req.event?.title || req.event_id}
                              </TableCell>
                              <TableCell>{req.event?.city || "-"}</TableCell>
                              <TableCell>{req.event?.date || "-"}</TableCell>
                              <TableCell>
                                {new Date(req.invited_at).toLocaleString()}
                              </TableCell>
              ...
              <FadeIn delay={100}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription className="text-black">
                      View revenue for current/ongoing events,
                      previous/completed events, and overall totals
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {(() => {
                      ...
                      const groupByEventRevenue = (bookingsList) => {
                        ...
                        bookingsList.forEach((booking) => {
                          ...
                          eventRevenueMap.get(eventId).revenue +=
                            booking.amount;
                        });
                        return Array.from(eventRevenueMap.values()).sort(
                      ...
                      const groupedCurrent =
                        groupByEventRevenue(currentEventBookings);
                      const groupedPrevious = groupByEventRevenue(
                        previousEventBookings
                      );
                      const totalCurrentRevenue = groupedCurrent.reduce(
                        (sum, e) => sum + e.revenue,
                        0
                      );
                      const totalPreviousRevenue = groupedPrevious.reduce(
                        (sum, e) => sum + e.revenue,
                        0
                      );
                      const totalRevenue =
                                  ...
                                  {groupedPrevious.length === 0 ? (
                                    <TableRow>
                                      <TableCell
                                        ...
                                        className="text-center text-muted-foreground py-4"
                                      >
                                        No revenue for previous/completed
                                        events.
                                      </TableCell>
                                    </TableRow>
                                  ) : (
            ...
            <TabsContent value="event-types">
                          ...
                          <TableBody className="text-black">
                            {eventTypes.map((eventType) => (
                              ...
                              >
                                ...
                                <TableCell className="text-2xl text-black">
                                  {eventType.icon || "🎭"}
                                </TableCell>
                                <TableCell>
                                  {eventType.image_url ? (
              ...
              <Dialog
                open={isEventTypeEditDialogOpen}
                onOpenChange={setIsEventTypeEditDialogOpen}
              >
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
            ...
            <TabsContent value="banners">
              ...
              <Dialog
                ...
                onOpenChange={setIsBannerEditDialogOpen}
              >
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
            ...
            <TabsContent value="testimonials">
                ...
                <FadeIn delay={50}>
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Review and approve or reject new testimonials submitted
                        by users.
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                                  ...
                                  <TableCell>
                                    <div className="flex items-center gap-1">
                                      {Array.from({
                                        ...
                                        <Star
                                          key={i}
                                          className="h-3 w-3 fill-yellow text-yellow"
                                        />
                                      ))}
                                    </div>
                                  </TableCell>
                          ...
                          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                            ...
                            <div className="p-4">
                              <p className="text-muted-foreground">
                                Use the feedback form on the main site to create
                                testimonials.
                              </p>
                            </div>
                          </DialogContent>
              ...
              <Dialog
                open={isTestimonialEditDialogOpen}
                onOpenChange={setIsTestimonialEditDialogOpen}
              >
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
              ...
              <FadeIn delay={100}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription className="text-black">
                      View and manage all user bookings, separated by event
                      status
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        {(() => {
                          ...
                          const groupByEvent = (bookingsList) => {
                            ...
                            bookingsList.forEach((booking) => {
                              ...
                              eventTicketMap.get(eventId).count +=
                                booking.tickets;
                            });
                          ...
                          const currentEventBookings = bookings.filter(
                            (b) =>
                              b.event?.date && new Date(b.event.date) >= now
                          );
                          const previousEventBookings = bookings.filter(
                            (b) => b.event?.date && new Date(b.event.date) < now
                          );
                          const groupedCurrent =
                            groupByEvent(currentEventBookings);
                          const groupedPrevious = groupByEvent(
                            previousEventBookings
                          );
                          return (
                                      ...
                                      {groupedCurrent.length === 0 ? (
                                        <TableRow>
                                          <TableCell
                                            ...
                                            className="text-center text-muted-foreground py-4"
                                          >
                                            No bookings for current/ongoing
                                            events.
                                          </TableCell>
                                        </TableRow>
                                      ) : (
                      ...
                      <>
                        ...
                        {(() => {
                          ...
                          const currentBookings =
                            bookings.filter(isCurrentEvent);
                      ...
                      <CardTitle className="flex justify-between items-center">
                        ...
                        <Dialog>
                          ...
                          <DialogContent>
                            ...
                            <div className="space-y-4">
                              <Label htmlFor="invite-host-email">
                                Host Email
                              </Label>
                              <Input
                                ...
                                onChange={(e) =>
                                  setHostInviteEmail(e.target.value)
                                }
                              />
                              <Button
                        ...
                        <div className="space-y-4">
                          {hostInvitations.length === 0 ? (
                            <div className="text-center py-8">
                              ...
                              <Button
                                onClick={() => setIsHostInviteDialogOpen(true)}
                              >
                                <Plus className="mr-2 h-4 w-4" />
                                Invite First Host
                              </Button>
              ...
              <FadeIn delay={100}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      See which host is managing which event, tickets sold,
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              Automatically mark tickets as attended for events
                              that have ended
                            </p>
                          </div>
      ...
      {/* Invite Host Dialog */}
      <Dialog
        open={isHostInviteDialogOpen}
        onOpenChange={setIsHostInviteDialogOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Invite New Host</DialogTitle>
